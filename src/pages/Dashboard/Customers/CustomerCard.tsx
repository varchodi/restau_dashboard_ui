...
const CustomerCard = ({currentCustomer, setCurrentCustomer, customer ,open}: {
  ...
  return (
    <article
      ...
      className={`flex  items-center gap-6 px-4 py-2 ${currentCustomer===customer.id?"bg-gray-300 shadow-md":"hover:bg-gray-300 hover:dark:bg-slate-700"}  cursor-pointer`}>
            <input className="scale-125 " type="checkbox" name="select-all" id="all" />
            <div className="flex justify-start min-w-[120px] items-center">
                <img className="w-8 h-8 rounded-full" src={customer.imgurl} alt="profile" />
                <div className="pl-2">
